...
def SuccessfulSignInRedirect(next, modal, request, close=False):
    modal_redirect_template = request.session.get("modal_redirect_template", "includes/modal_close.html")
...
def register_view(request, message='', username='', password='', email='', template='user/register.html', modal=False):
    modal = request.REQUEST.get("modal", False)
    close = request.REQUEST.get("close", False)
    if modal == "true":
        template = 'user/register_modal.html'
...
def get_login_buttons(request):
    next = request.REQUEST.get('next', reverse('home'))
    t = get_template('includes/login_buttons.html')
    to_return = {
